...
-- Я решила рассматривать уникальные категории, потому что в моем понимании изначально при регистрации 
-- на маркетплейсе у тебя есть n категорий для выбора. Если у sellera есть две и более одинаковых категории,
-- то возможно он просто перерегистрировал заново один и тот же товар, но это все же остается одной категорией
-- например, у товара изменились характеристики, селлер не разобрался как их поменять/или нельзя поменять,
-- поэтому он решил перерегистрировать товар. Поэтому мне кажется нечестным считать селлера poor или rich, если он наплодил кучу регистраций с одинаковой категорией,
--ассортимент товара у него при этом небольшой
